...
export function TopBar({ onMenuToggle }: Props) {
  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-border/60 bg-white/80 px-4 shadow-sm backdrop-blur-sm lg:px-6">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button className="flex items-center gap-2.5 rounded-2xl border border-border/60 bg-white px-3 py-1.5 shadow-xs transition-shadow hover:shadow-sm">
            <div
            ...
            <svg className="ml-1 hidden h-3.5 w-3.5 text-gray-400 md:block" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
        ...
        <DropdownMenuContent align="end" className="w-52 rounded-2xl p-1.5 shadow-xl">
          <DropdownMenuLabel className="px-3 py-2">
            <p className="text-sm font-bold text-gray-900">{user ? `${user.first_name} ${user.last_name}` : '…'}</p>
