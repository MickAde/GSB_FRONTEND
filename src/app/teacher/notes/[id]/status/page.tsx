...
const messages: Record<string, { title: string; description: string }> = {
  PENDING_OCR: {
    ...
    description: 'Reading your document. This takes 15–60 seconds depending on file size.',
  },
  AWAITING_STUDENT_APPROVAL: {
    title: 'Text extracted — taking you to review…',
    description: 'Check the OCR output for any errors before AI generates your summary. Redirecting now.',
  },
  PROCESSING_AI: {
    title: 'AI is generating your summary…',
    description: 'Claude is analysing your notes. Usually under a minute.',
  },
  READY: {
    title: 'Note is ready!',
    description: 'Processed and ready to use in conformity reports. Taking you there now.',
  },
  FAILED: {
    title: 'Processing failed',
    description: 'Something went wrong. See the error below, or try uploading again.',
  },
};
...
export default function TeacherNoteStatusPage(props: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (status === 'AWAITING_STUDENT_APPROVAL') {
      const t = setTimeout(() => router.push(`/teacher/notes/${id}/review`), 1500);
      return () => clearTimeout(t);
    }
    if (status === 'READY') {
      const t = setTimeout(() => router.push(`/teacher/notes/${id}`), 2000);
      return () => clearTimeout(t);
    }
  }, [status, id, router]);
  ...
  return (
    <div className="mx-auto flex max-w-lg flex-col items-center space-y-8 pt-10">
      ...
      {status === 'AWAITING_STUDENT_APPROVAL' && (
        <Link href={`/teacher/notes/${id}/review`}>
          <Button className="bg-amber-500 text-white hover:bg-amber-600">Review Text →</Button>
        </Link>
      )}
      {status === 'READY' && (
        <Link href={`/teacher/notes/${id}`}>
          <Button className="bg-green-600 hover:bg-green-700">View Note →</Button>
        </Link>
      )}
      {isFailed && (
        <div className="flex flex-col items-center gap-3 w-full">
          <Link href={`/teacher/notes/${id}/review`} className="w-full">
            <Button className="w-full bg-amber-500 text-white hover:bg-amber-600">Edit Text &amp; Retry AI →</Button>
          </Link>
          <Link href="/teacher/notes/upload" className="w-full">
            <Button variant="outline" className="w-full">Upload a Different Note</Button>
          </Link>
        </div>
      )}

      <Link href="/teacher/notes" className="text-sm text-primary hover:underline">← Back to My Notes</Link>
    </div>
  );
}
