...
const messages: Record<string, { title: string; description: string }> = {
  PENDING_OCR: {
    title: 'Extracting text from your note…',
    description: 'Our OCR engine is reading your document. Large or scanned files can take up to a minute.',
  },
  AWAITING_STUDENT_APPROVAL: {
    title: 'Text extracted — taking you to review…',
    description: 'Check the extracted text for errors before we pass it to AI. Redirecting you now.',
  },
  PROCESSING_AI: {
    title: 'AI is generating your summary…',
    description: 'Claude is analysing your notes and building a structured summary. Usually under a minute.',
  },
  READY: {
    title: 'Your summary is ready!',
    description: 'All done. Taking you to your AI-powered summary now.',
  },
  FAILED: {
    title: 'Processing failed',
    description: 'Something went wrong. See the error below, or try uploading a new note.',
  },
};
...
export default function NoteStatusPage(props: { params: Promise<{ id: string }> }) {
  ...
  // Auto-redirect when a terminal state is reached
  useEffect(() => {
    if (status === 'AWAITING_STUDENT_APPROVAL') {
      const t = setTimeout(() => router.push(`/student/notes/${id}/review`), 1500);
      return () => clearTimeout(t);
    }
    if (status === 'READY') {
      const t = setTimeout(() => router.push(`/student/notes/${id}`), 2000);
      return () => clearTimeout(t);
    }
  }, [status, id, router]);
  ...
  return (
    <div className="mx-auto flex max-w-lg flex-col items-center space-y-8 pt-10">
      ...
      {/* ── Manual fallback buttons (in case auto-redirect doesn't fire) ── */}
      ...
      {isFailed && (
        <div className="flex flex-col items-center gap-3 w-full">
          <Link href={`/student/notes/${id}/review`} className="w-full">
            <Button className="w-full bg-amber-500 text-white hover:bg-amber-600">
              Edit Text &amp; Retry AI →
            </Button>
          </Link>
          <Link href="/student/notes/upload" className="w-full">
            <Button variant="outline" className="w-full">Upload a Different Note</Button>
          </Link>
        </div>
      )}

      <Link href="/student/notes" className="text-sm text-primary hover:underline">
        ← Back to My Notes
      </Link>
    </div>
  );
